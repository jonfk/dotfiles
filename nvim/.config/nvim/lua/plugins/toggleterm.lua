return {
	{
		...
		config = function(_, opts)
			...
			vim.keymap.set({ "n", "t" }, "<C-.>", function()
				-- Get current buffer and check if it's a toggleterm buffer
				local current_buf = vim.api.nvim_get_current_buf()
				local buf_name = vim.api.nvim_buf_get_name(current_buf)
				local is_toggleterm = string.match(buf_name, "term://.*toggleterm#%d+")
				local is_terminal = vim.bo.buftype == "terminal"

				if is_toggleterm then
					-- If it's a toggleterm buffer, close it
					vim.cmd("ToggleTerm")
				elseif is_terminal then
					-- If it's a regular terminal buffer, switch to alternate file
					vim.cmd("buffer #")
				end
			end, { desc = "Close Terminal or Switch to Alt File" })
		end,
	},
	{
